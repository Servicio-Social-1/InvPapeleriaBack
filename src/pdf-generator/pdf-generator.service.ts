...
import { TextCell } from './interfaces/text-cell.interface';
...
export class PdfGeneratorService {
    constructor(
        ...
        private articleExitService: ArticleExitService,
    ) { }

    generatePdf(createPdfGeneratorDto: CreatePdfGeneratorDto): Promise<Buffer> {
        const options = {
            entry: () => {
                return this.generateCustomArticleExitReportPDF(
    ...
    async generateInventoryReport(): Promise<Buffer> {
        ...
        const pdfBuffer: Buffer = await new Promise((resolve) => {
            ...
            const path = join(__dirname, '../assets/img/gobierno.png');
            ...
            inventoryData.forEach((table, index) => {
                doc.table(table as any, table.options);
            });
            ...
            // doc.on('pageAdded', () => {
            //     doc.image(path, 100, 0);
            // });
        });
        return pdfBuffer;
    }

    async generateArticleExitReport(
        createPdfGeneratorDto: CreatePdfGeneratorDto,
    ): Promise<Buffer> {
        const articleExit = await this.articleExitService.findById(
            createPdfGeneratorDto.id,
        );
        ...
        const pdfBuffer: Buffer = await new Promise((resolve) => {
            ...
            const fontBoldPath = join(__dirname, '../assets/fonts/SourceSansPro-Light.otf');
            doc.registerFont('Sans-Pro', fontBoldPath);

            const fontPath = join(__dirname, '../assets/fonts/SourceSansPro-Bold.otf');
            ...
            const inventoryData = [
                {
                    ...
                    datas: articleExit.articleExitDetail.map(
                        ({ article, amount, ...rest }: ArticleExitDetail) => {
                            return {
                                id: article.id,
            ...
            for (let index = 0; index < inventoryData[0].datas.length; index++) {
                ...
                if((index % 24 === 0) &&  inventoryData[0].datas.length > 23) {
                    ...
                    doc.x = 30
                    doc.image(path, 55, 5, {height: 115, width: 490});
                    doc.table({headers: inventoryData[0].headers, datas: inventoryData[0].datas.slice(index, index + 24)} as any);
                    ...
                    doc.x = 30
                    doc.image(path, 55, 5, {height: 115, width: 490});
                    doc.table({headers: inventoryData[0].headers, datas: inventoryData[0].datas.slice(0, 24)} as any);  
    ...
    async generateCustomArticleExitReportPDF(
        createPdfGeneratorDto: CreatePdfGeneratorDto,
    ): Promise<Buffer> {
        const pdfBuffer: Buffer = await new Promise((resolve) => {
    ...
    private createCell({ doc, x, y, width, height }: Cell) {
    ...
    private descriptions(doc: PDFKit.PDFDocument) {
    ...
    private articles(doc: PDFKit.PDFDocument) {
        // Filas
    ...
    private signatures(doc: PDFKit.PDFDocument) {
